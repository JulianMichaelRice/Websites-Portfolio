...
function daily(xml) {
    ...
    for (var i = parentTag.length-1; i >= 0; i--) {
        ...
        avatar = parentTag[i].getAttribute("name");
...
function ranking(xml) {
    ...
    var show_max_num = 200;
    var show_total = (x.length > show_max_num) ? show_max_num : x.length;
    for (i = 0; i < show_total; i++) { //x.length
