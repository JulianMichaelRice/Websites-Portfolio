...
$search_fields = array('area', 'name', 'mob', 'trophy', 'group_');
...
try {
    ...
    $query = 'SELECT id, timestamp, group_, trophy, profile, name, mob, value, area, time FROM';
    ...
    foreach ($search_fields as $search_field) {
        if (isset($_GET[$search_field])) {
            ...
            //$query = $query . ' `' . $search_field . '` LIKE "%' . $_GET[$search_field] . '%" ';
    ...
    //foreach ($stmt as $row)
    //    {
    //    $hunting[$row['location']][$row['product_code']] = $row['quantity'];
    //    }
}
